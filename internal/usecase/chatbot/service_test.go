...
func newStubMessageDeduplicator() *stubMessageDeduplicator {
	return &stubMessageDeduplicator{
...
func TestProcessIncomingMessageSkipsDuplicateMessageID(t *testing.T) {
	...
	if err := service.ProcessIncomingMessage(context.Background(), message); err != nil {
...
func TestProcessIncomingMessageReleasesMessageIDOnFailure(t *testing.T) {
	...
	sender := &stubMessageSender{}
	deduplicator := newStubMessageDeduplicator()
	replyGenerator := &sequenceReplyGenerator{
		replies: []string{"", "assistant reply"},
		errors:  []error{errors.New("gemini failure"), nil},
	}
	...
	firstErr := service.ProcessIncomingMessage(context.Background(), message)
